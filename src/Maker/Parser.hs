...
lockStateParser :: Parser LockState
lockStateParser = do
  state <- many letterChar
  case state of
    "Locked"   -> pure Locked
    "Unlocked" -> pure Unlocked
    invalid    -> fail $ "Invalid lock state: " ++ invalid ++ " (expected Locked or Unlocked)"

lockParser :: Parser Lock
lockParser = do
  void . string $ "-- Lock:"
  void newline
  keyItems <- indentedPropertyListParser "Keys" entityRefParser
  failMsg <- indentedTextPropertyParser "FailMsg"
  successMsg <- indentedTextPropertyParser "SuccessMsg"
  state <- indentedPropertyParser "State" lockStateParser
  pure $ Lock
    { _lockKeyItems = map _entityRefEntityId keyItems
    , _lockFailMsg = failMsg
    , _lockSuccessMsg = successMsg
    , _lockState = state
    }
  where
    indentedPropertyParser :: Text -> Parser a -> Parser a
    indentedPropertyParser lbl p = do
      void . string $ "--   "
      void . string $ lbl
      void . string $ ": "
      prop <- p
    ...
    indentedTextPropertyParser :: Text -> Parser Text
    indentedTextPropertyParser lbl = do
      void . string $ "--   "
      void . string $ lbl
      void . string $ ": "
      txt <- T.pack <$> manyTill latin1Char newline
      pure txt

    indentedPropertyListParser :: Text -> Parser a -> Parser [a]
    indentedPropertyListParser lbl p = do
      void . string $ "--   "
      void . string $ lbl
      void . string $ ": "
...
containerLockParser :: Parser ContainerLock
containerLockParser = do
  void . string $ "-- Lock:"
  void newline
  keyItems <- indentedPropertyListParser "Keys" entityRefParser
  failMsg <- indentedTextPropertyParser "FailMsg"
  successMsg <- indentedTextPropertyParser "SuccessMsg"
  unlockedDesc <- indentedTextPropertyParser "UnlockedDesc"
  state <- indentedPropertyParser "State" lockStateParser
  pure $ ContainerLock
    ...
    , _containerLockUnlockedDescription = unlockedDesc
    }
  where
    indentedPropertyParser :: Text -> Parser a -> Parser a
    indentedPropertyParser lbl p = do
...
gameObjectParser = do
  ...
  gameObject <- case gameObjectType of
    ...
    ContainerType -> do
      containerSize <- propertyParser "Size" intParser
      containerItems <- propertyListParser "Items" entityRefParser
      containerLock <- optional containerLockParser
      pure
        $ ObjectContainer
        $ Container
        { _containerSize = containerSize
        , _containerItems = M.fromList . map entityRefToMap $ containerItems
        , _containerLock = containerLock
        }
  let g = GameObject
...
worldParser = do
  ...
  pure $ World
    ...
    , _worldPlayerRoom = EntityId 1  -- Start at first room
    , _worldPlayerInventory = mempty
    , _worldPlayerVerbs = defaultVerbAliasMap  -- Load default verbs
